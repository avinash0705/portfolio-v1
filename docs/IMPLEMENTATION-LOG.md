...
**Marker row restructured to `01 — •`; connector switched to spaced dots; trailing dash added.** A second annotated reference image was provided specifically to settle three remaining differences from the target, after the dashed-connector attempt above turned out not to match what was actually shown.

- **Marker row**: changed from a vertical stack (number above dot) to a horizontal row — number, a short dash, then the dot — using a fixed three-column CSS grid (`24px`/`16px`/`24px`) rather than flex/gap, so the connector line below has a known, computed horizontal offset (the dot column's centre) to align to, instead of an estimated one.
- **Connector**: `border-dashed` replaced with a `repeating-linear-gradient` background — CSS `border-dashed`/`border-dotted` both render tight, closely-packed marks regardless of border-width, which didn't match the reference's clearly spaced round dots. The gradient gives real, controllable gaps. Trade-off, disclosed rather than hidden: the gradient's colour swap (upcoming → reached) is instant, not transitioned — CSS doesn't interpolate `background-image` smoothly, and building a cross-fade of two overlaid gradients wasn't judged worth a second technique for a decorative connector.
- **Trailing dash**: `SectionHeading` gained an optional `indexMark` prop — an `aria-hidden` dash rendered after the title, mirroring the marker row's own dash, enabled only on the five Homepage sections that actually have a Section Index marker to pair with (never a page-wide default, so a future page without Section Index doesn't inherit an orphaned dash). Hero's role line gets the same treatment unconditionally, since Hero has no variants and always pairs with Section Index on the one page it exists on.
- **Verified**: format/typecheck/lint/build clean; real production build confirmed the trailing dash present on all five `SectionHeading` calls plus Hero's role line (six total), `aria-hidden` in every case, and that each heading's own accessible text (e.g. "Highlights") is unaffected. The marker row's actual visual alignment and dot spacing remain unverified — no browser in this environment; column widths and the connector's offset are reasoned estimates, not pixel-checked against a rendered page.
