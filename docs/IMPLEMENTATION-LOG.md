...
- **Deliberately not implemented — flagged, not decided unilaterally**: the reference direction also asked for a premium editorial serif on the large headline (Newsreader/Instrument Serif/Fraunces). `006-design-system.md`, Section 4 currently states plainly that "One typeface family carries all interface and prose text... Visual contrast comes from weight and size, not from mixing multiple typefaces." Adding a second, non-monospace typeface directly contradicts that sentence as written. This is the same category of tension as the container/illustration/delight-cap conflicts already resolved earlier in this document — it needs a decision and, if approved, a `006` amendment, not a silent implementation. The headline currently uses the existing sans family at significantly larger size/tighter tracking to reach for the same visual weight the screenshot shows, pending that decision.
